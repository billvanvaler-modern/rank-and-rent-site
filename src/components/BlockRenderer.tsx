...
export function BlockRenderer({
  ...
  /** Only used by the contact_info case, to let its form (when enabled) submit a real lead. Omit on any page that doesn't render a contact_info block. */
  siteId?: string;
}) {
  switch (block.block_type) {
    ...
    case "contact_info":
      return (
        <ContactInfo
          content={block.content as unknown as ContactInfoContent}
          siteId={siteId}
...
function ContactInfo({
  ...
  return (
    <section className={section}>
      ...
      {content.form_enabled && siteId && <ContactForm siteId={siteId} pageId={pageId} />}
      {content.form_enabled && !siteId && (
        <p className="text-slate-400 text-sm">
          (Contact form couldn&apos;t load -- for now, use the phone number / email in the footer.)
        </p>
      )}
    </section>
  );
