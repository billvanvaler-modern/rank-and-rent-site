...
// Service and location pages both resolve at the root now (e.g.
// /roof-replacement-noblesville), matching the reference site's flat,
// keyword+city URL pattern rather than nesting under /services or
// /locations -- those hub paths still exist as their own static routes for
// the nav/index page, they just aren't a URL prefix for individual pages
// anymore. `pages.slug` is unique per site across all page_types (see
// 001_initial_schema.sql), so trying "service" then "location" here can
// never double-match.
async function loadPage(slug: string) {
  const site = await getCurrentSite();
  if (!site) return null;
  const page =
    (await getPageByTypeAndSlug(site.id, "service", slug)) ??
    (await getPageByTypeAndSlug(site.id, "location", slug));
